...
import { SemisterRegistrationModel } from "../semisterRagistration/semisterRagistration.model";
...
const createOfferedCourseIntoDB = async (payload: TOfferedCourse) => {
  ...
  const isSemesterRegistrationExits =
    await SemisterRegistrationModel.findById(semesterRegistration);
  ...
  const isAcademicDepartmentExits =
    await AcademicDepertmantModel.findById(academicDepartment);
  ...
  const isDepartmentBelongToFaculty = await AcademicDepertmantModel.findOne({
...
const deleteOfferedCourseFromDB = async (id: string) => {
  ...
  const semesterRegistrationStatus =
    await SemisterRegistrationModel.findById(semesterRegistation).select(
      "status"
    );
